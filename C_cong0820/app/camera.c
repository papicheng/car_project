...
#define COLUMN 180       //列
#define ROW    120      //行
#define ROWbegin    30      //开始采集行
#define ROWend    120      //结束采集行
...
float proportion_lightpoint; //亮点比例
void Calculate_Middle_Point(void)
{
  int i,j,k=0,prok=0,minj=COLUMN,maxj=0,mini=ROWend,maxi=ROWbegin;
  int pointy,pointx;
  int picture_maxj=0,picture_minj=COLUMN,picture_maxi=0,picture_mini=ROWend;
  int picture_lightpoint_number=0;
  uint8 nopoint_row=0;
  uint8 isolate_flag=0;
  
  
  if(WhichBuffer == 2)//正在传图2
  {
     lock=1;//锁图1，处理图1
     for (i=ROWnum-1;i>0 && isolate_flag==0;i--)
     {
      //  for (j=COLUMN-1;j>0;j--)//从下往上扫描
       for (j=140;j>40 && isolate_flag==0;j--)//从下往上扫描
        {
          if (nopoint_row<=5)//还没有分离点
          {
        ...
        if(k>prok )
        {
          if (i>35 && k>15 || i<=35 && i>=14 && k>4 && k<15|| i<14 && k>2)
          {
            prok=k;
            pointy=i;
            pointx=(minj+maxj)/2;
        ...
        else if (k<prok && k==0) //统计非连通行数
        {
          nopoint_row++;
        }
        picture_lightpoint_number+=k;
        k=0;
     }//一幅图处理完毕
    /**********计算块区比例**********/
     proportion_lightpoint = 1.0*picture_lightpoint_number / (picture_maxi-picture_mini+1) / (picture_maxj-picture_minj+1);
  }
  ...
  if (prok==0||prok>50||isolate_flag||proportion_lightpoint<=0.5||picture_maxj-picture_minj+1>30||picture_maxi-picture_mini+1>30)
    NOpoint=1;
  else 
  {
    NOpoint=0;
