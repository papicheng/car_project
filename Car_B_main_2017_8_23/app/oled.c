...
//OLED显示屏
//定义输出
#define LED_SCLK(x) LPLD_GPIO_Output_b(PTC, 16, x)// d0
#define LED_SDA(x)  LPLD_GPIO_Output_b(PTC, 17, x)//d1
#define LED_RST(x)  LPLD_GPIO_Output_b(PTC, 18, x)//复位低能电平
#define LED_DC(x)   LPLD_GPIO_Output_b(PTC, 19, x)//偏置常低
//OLED显示函数
...
void led_gpio_int(void)
{
  ...
  led_Port_init.GPIO_PTx = PTC;
  led_Port_init.GPIO_Dir = DIR_OUTPUT;
  led_Port_init.GPIO_Pins = GPIO_Pin16|GPIO_Pin17|GPIO_Pin18|GPIO_Pin19;
